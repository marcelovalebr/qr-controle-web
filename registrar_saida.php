    ...
    if (empty($cpf)) {
        ...
        if (mysqli_num_rows($result_check) == 0) {
            ...
            if (mysqli_num_rows($result_check_entrada_sem_saida) == 0) {
                $msg = "Nenhuma entrada em aberto para $nome.";
            } else {
                // Registrar a saída
                $sql = "INSERT INTO registros (cpf, acao) VALUES ('$cpf', 'saida')";
                if (mysqli_query($conn, $sql)) {
                    $success = true;
                    $msg = "Saída registrada com sucesso para $nome.";
                } else {
                    $msg = "Erro ao registrar saída: " . mysqli_error($conn);
                }
            }
        }
    }
...
<head>
    ...
    <title>Registrar Saída - QGControle</title>
    ...
    <script>
        ...
        function checkCPF() {
            ...
            if (cpf.length == 11) { // Verifica se o CPF tem 11 dígitos
                ...
                xhr.open("POST", "verificar_saida.php", true);
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState === XMLHttpRequest.DONE) {
                        if (xhr.status === 200) {
                            ...
                            } else if (response.status === "no_entry") {
                                nomeField.value = "Nenhuma entrada em aberto.";
                            }
                        }
                    }
                };
